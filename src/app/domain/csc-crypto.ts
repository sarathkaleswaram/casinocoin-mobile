...
export class CSCCrypto {
  ...
  private static PASSPHRASE_WORD_COUNT = 10;
  ...
  constructor(password: string | Array<string>, salt: string);
  constructor(password?: string | Array<string>, salt?: string) {
    ...
    let saltHash = crypto.createHash("sha512");
    if(salt !== undefined && salt.length > 0){
      saltHash.update(salt);
      let saltString = saltHash.digest("base64");
      this.passwordSalt = Buffer.from(saltString, 'utf8');
    } else {
      this.passwordSalt = new Buffer(crypto.randomBytes(this.SALT_LENGTH));
    }
  }
  ...
  encrypt(inputValue:string) {
    ...
    // encrypt password
    let key:Buffer = new Buffer(crypto.pbkdf2Sync(this.passwordKey, this.passwordSalt, this.pbkdf2Rounds, this.pbkdf2KeyLength, this.pbkdf2Digest));
  ...
  decrypt(encryptedInput:string) {
    ...
    let iv = rawData.slice(0, this.IV_LENGTH);
    let authTag = rawData.slice(this.IV_LENGTH, this.IV_LENGTH + 16);
    let data = rawData.slice(this.IV_LENGTH + 16);

    // decrypt password
    let key = crypto.pbkdf2Sync(this.passwordKey, this.passwordSalt, this.pbkdf2Rounds, this.pbkdf2KeyLength, this.pbkdf2Digest);
  ...
  generateKeyPair(sequence:number): CSCKeyPair {
    // concat path with mnemonic hash
    let pathWithHash = 'm/0/'+ sequence + '/' + this.passwordKey;
    let reHashedHash =  crypto.createHash("sha512");
    reHashedHash.update(pathWithHash);
    // generate hex entropy
    let entropyHex = crypto.pbkdf2Sync(reHashedHash.digest("base64"), this.passwordSalt, this.pbkdf2Rounds, this.pbkdf2KeyLength, this.pbkdf2Digest).toString('hex');
    // convert hex string to array buffer
    var entropyArray = new Uint8Array(entropyHex.match(/[\da-f]{2}/gi).map(function (h) {
      return parseInt(h, 16)
    }));
    let entropy:any = {entropy: entropyArray};
    // generate seed
    let seed:string	=	generateSeed(entropy);
    // derive keypair
    let keypair:any	=	deriveKeypair(seed);
    let newKeyPair:CSCKeyPair	=	{ seed: seed, publicKey: keypair.publicKey, privateKey: keypair.privateKey, accountID: deriveAddress(keypair.publicKey)};
    return newKeyPair;
  }
